...
This version wraps the login page's `useSearchParams()` usage in a React `Suspense` boundary, which is required by newer Next.js builds on Vercel.


## Conservative build version

This package pins Next.js to 14.2.23 and React to 18.3.1 to avoid Next 15 / React 19 build incompatibilities on Vercel.
It also includes `.npmrc` with `legacy-peer-deps=true`.


## Login build fix

The `/login` page no longer uses `useSearchParams()`. The `next` path is read by the server page and passed into a client component. This avoids the Next.js prerender error requiring a Suspense boundary.
