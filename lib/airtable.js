...
export function normalizeDynamicFields(input) {
  ...
  for (const [key, value] of Object.entries(input || {})) {
    if (key === "id" || key === "Medication") continue;
...
export function normalizePendingChangeFields(input) {
  const fields = {
    ...
    "Change Summary": String(input["Change Summary"] || input.changeSummary || "").trim(),
    "Submitted By": String(input["Submitted By"] || input.submittedBy || "").trim(),
    "Reviewed By": String(input["Reviewed By"] || input.reviewedBy || "").trim()
  };
