...
function formatDetailValue(value) {
  ...
  // Hide raw Airtable linked-record IDs like:
  // recvGiWNBvS286kRS
  // recXXXXXXXXXXXXXX
  if (/^rec[a-zA-Z0-9]{10,}$/.test(text)) {
    return "";
  }
...
const HOME_INFUSION_DETAIL_FIELDS = [
  ...
  { label: "REMs Program", aliases: ["REMs Program", "REMS Program", "REMS", "REMs Program "] },
...
function findDetailField(details, aliases) {
  ...
  // Second: relaxed contains match, useful if Airtable columns have extra spaces, punctuation, or suffix text.
  for (const [key, value] of entries) {
...
function getHomeInfusionDetails(route) {
  ...
  // Always return every requested row, even if the Airtable value is blank.
  // This prevents rows after Pre-medications from disappearing due to a blank value or small field-name mismatch.
...
export default function Home() {
  ...
  return (
    <main>
      ...
      <p className="versionMarker">Home Infusion update v4.1 · HI details below table v4.9 · Removed HI status row v5.0 · Detail-only HI rows v5.1 · Strict HI details v5.2 · Force all selected HI rows v5.6 · Clean Airtable IDs v5.7 · Force all selected HI rows v5.6 · Clean Airtable IDs v5.7 · Force all selected HI rows v5.6 · Clean Airtable IDs v5.7 · Force all selected HI rows v5.6 · Clean Airtable IDs v5.7</p>
    </main>
  );
}
