...
function getHomeInfusionDetails(route) {
  ...
  return HOME_INFUSION_DETAIL_FIELDS
    .map(key => ({ key, value: formatDetailValue(route.details[key]) }))
...
function getVisibleRouteRows(record, selectedLocation) {
  ...
  // Hide the synthetic UMM Formulary/Home Infusion row from the top Route/Status table.
  // The same UMM Formulary data is still shown below in the Home Infusion Details box.
  return routes.filter(route => route.source !== "UMM Formulary");
}
...
export default function Home() {
  ...
  return (
    <main>
                  ...
                  <div className="routeList">
                    ...
                    <div className={selectedLocation ? "routeTable noLocationColumn" : "routeTable"}>
                      ...
                      {getVisibleRouteRows(record, selectedLocation).length === 0 && (
                        <div className="routeTableRow noLocationColumn">
                          <span>No route listed</span>
                          <span>Status not listed</span>
                        </div>
                      )}
                      ...
                      <div className="homeInfusionDetails">
                        ...
                        {getHomeInfusionDetailRows(record, selectedLocation).map(row => (
                          <div className="homeInfusionDetailCard" key={row.id}>
                            <div className="homeInfusionDetailSummary">
                              <span>{row.route}</span>
                              <em>{row.status}</em>
                            </div>
      ...
      <p className="versionMarker">Home Infusion update v4.1 · HI details below table v4.9 · Removed HI status row v5.0</p>
    </main>
  );
}
