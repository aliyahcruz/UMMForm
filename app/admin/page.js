...
const ROUTE_FIELDS = ["Medication", "Medication ID", "Generic Name", "Route", "UMMS Formulary Status", "Notes", "Therapeutic Interchanges"];
...
export default function AdminPage() {
  ...
  const routeFuse = useMemo(() => new Fuse(routes, {
    keys: ["Medication ID", "Generic Name", "Route", "UMMS Formulary Status", "Notes", "Therapeutic Interchanges"],
    threshold: 0.35, ignoreLocation: true, minMatchCharLength: 2
  }), [routes]);
  ...
  return (
    <main>
      <section className="home adminHome">
        ...
        <p className="subtitle">Manage Medications and Routes tables</p>
        ...
        <div className="adminTabs">
          ...
          <button type="button" className={tab === "routes" ? "active" : ""} onClick={() => setTab("routes")}>Routes</button>
        </div>
      </section>
      ...
      {tab === "routes" && (
        <>
          <section className="editor">
            <h2>{selectedRoute ? "Edit route/status" : "Add route/status"}</h2>
            <form onSubmit={event => save("route", event)}>
              ...
              <label>Edit key, if configured<input value={editKey} onChange={event => setEditKey(event.target.value)} /></label>
              <button type="submit" disabled={saving}>{saving ? "Saving..." : "Save route/status"}</button>
          ...
          <section className="results">
            {routeResults.map(record => (
              <article className="result" key={record.id}>
                ...
                <button type="button" onClick={() => { setSelectedRoute(record); setRouteForm(getFields(record, ROUTE_FIELDS)); }}>Edit route/status</button>
              </article>
            ))}
          </section>
        </>
