...
function editableFields(record, preferredFields = []) {
  const existing = Object.keys(record || {}).filter(key => !HIDDEN_EDIT_FIELDS.has(key));
...
function RecordEditor({ title, endpointBase, tableName, record, editKey, preferredFields = [], onSaved, versionHistory = [] }) {
  ...
  async function pendChanges() {
    ...
    setMessage("Pending change saved for admin review only. It will NOT appear on the public front end until Save to Airtable is selected.");
    onSaved?.();
  }
...
export default function AdminPage() {
  ...
  return (
    <main>
            ...
            <section className="adminRelatedBlock">
              ...
              {pendingChanges.map(record => (
                <div className="versionHistoryItem" key={record.id}>
                  ...
                  {record["Fields JSON"] && <pre>{record["Fields JSON"]}</pre>}
                </div>
              ))}
            </section>
          )}
      ...
      <p className="versionMarker">Admin UMM-only v6.3</p>
    </main>
  );
}
